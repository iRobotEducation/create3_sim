...
IrIntensity::IrIntensity(std::shared_ptr<rclcpp::Node> &nh) : nh_(nh),
...
{
    auto ir_scan_sub_topics = irobot_create_toolbox::declare_and_get_parameter<std::vector<std::string>>("ir_scan_subscription_topics", nh_.get());
    auto ir_intensity_pub_topics = irobot_create_toolbox::declare_and_get_parameter<std::vector<std::string>>("ir_intensity_publish_topics", nh_.get());

    for (std::string topic : ir_scan_sub_topics)
    {
        ir_scan_sub_.push_back(nh_->create_subscription<sensor_msgs::msg::LaserScan>(
            topic,
    ...
    for (std::string topic : ir_intensity_pub_topics)
    {
        for (const std::string sensor : ir_intensity_sensors_)
        {
            ...
            {
                ir_intensity_pub_[sensor] = nh_->create_publisher<irobot_create_msgs::msg::IrIntensity>(
                                                topic,
                                                rclcpp::SensorDataQoS());
            }
        }
...
float IrIntensity::find_minimum_range(std::vector<float> ranges)
{
...
void IrIntensity::ir_scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr ir_msg)
{
    ...
    const double detection = std::min(find_minimum_range(ir_msg->ranges), ir_msg->range_max);
    ...
    ir_intensity_msg.value = static_cast<irobot_create_msgs::msg::IrIntensity::_value_type>(scaled_detection);
