...
void GazeboRosBumper::OnUpdate()
{
  ...
  for (int i = 0; i < contacts.contact_size(); ++i)
  {
    ...
    const ignition::math::Vector3d c_vec = gazebo::msgs::ConvertIgn(contacts.contact(i).position(0));
    ...
    {
      if (IsAngleBetween(relative_contact_angle_xy, bumper_zone.second.left_limit, bumper_zone.second.right_limit))
      {
...
void GazeboRosBumper::GzPoseCallback(ConstPosesStampedPtr &msg)
{
  ...
  msg_.header.stamp = gazebo_ros::Convert<builtin_interfaces::msg::Time>(bumper_->LastMeasurementTime());
  ...
  for (auto &pose : msg->pose())
  {
    ...
    if (pose.name() == "create3")
    {
      ...
      r_tf_w_ = ignition::math::Matrix4d(
          ignition::math::Pose3d(pose.position().x(), pose.position().y(), pose.position().z(), pose.orientation().w(),
