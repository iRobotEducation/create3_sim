...
void GazeboRosWheelDrop::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
{
  ...
  update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&GazeboRosWheelDrop::OnUpdate, this));
...
void GazeboRosWheelDrop::PublishState(const bool& state, const double& displacement, const gazebo::common::Time& current_time)
{
  ...
  pub_->publish(msg);
  RCLCPP_INFO_EXPRESSION(ros_node_->get_logger(), !state, "Wheel drop %s OFF: %.3f", name_.c_str(), displacement);
  RCLCPP_INFO_EXPRESSION(ros_node_->get_logger(), state, "Wheel drop %s ON: %.3f", name_.c_str(), displacement);
}
