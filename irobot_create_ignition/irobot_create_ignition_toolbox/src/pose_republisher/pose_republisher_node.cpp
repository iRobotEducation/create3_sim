                ...
                "right_wheel_joint"}
{
  ...
  std::string robot_pub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("robot_publisher_topic", this);
  std::string robot_sub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("robot_subscriber_topic", this);
  std::string mouse_pub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("mouse_publisher_topic", this);

  std::string dock_pub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("dock_publisher_topic", this);
  std::string dock_sub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("dock_subscriber_topic", this);
  std::string ir_emitter_pub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("ir_emitter_publisher_topic", this);
  std::string ir_receiver_pub_topic = irobot_create_toolbox::declare_and_get_parameter<std::string>("ir_receiver_publisher_topic", this);
...
void PoseRepublisher::robot_subscriber_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg)
{
  for (uint16_t i = 0; i < msg->transforms.size(); i++)
  {
    ...
    else if (msg->transforms[i].child_frame_id.find("mouse") != std::string::npos)
    {
      ...
      tf2::Transform mouse_world_pose = utils::static_link_wrt_global_frame(mouse_pose, last_robot_pose_);
    ...
    else if (msg->transforms[i].child_frame_id.find("ir_omni") != std::string::npos)
    {
      ...
      tf2::Transform receiver_world_pose = utils::static_link_wrt_global_frame(receiver_pose, last_robot_pose_);
      // Convert tf2 transform back to pose
...
void PoseRepublisher::dock_subscriber_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg)
{
  for (uint16_t i = 0; i < msg->transforms.size(); i++)
  {
    ...
    else if (msg->transforms[i].child_frame_id.find("halo_link") != std::string::npos)
    {
      ...
      tf2::Transform emitter_world_pose = utils::static_link_wrt_global_frame(emitter_pose, last_dock_pose_);
      // Convert tf2 transform back to pose
...
void PoseRepublisher::joint_state_subscriber_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
